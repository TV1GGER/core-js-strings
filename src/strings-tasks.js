...
function removeFirstOccurrences(str, value) {
  const indexWord = str.indexOf(`${value}`);
  let newStr1 = '';
  let newStr2 = '';
  if (value === '') {
    return str;
  ...
  if (str.includes(value)) {
    newStr1 += str.slice(0, indexWord);
    newStr2 += str.slice(indexWord + value.length, str.length);
    const result = newStr1.concat('', newStr2);
    if (result.includes('   ')) {
...
function removeLastOccurrences(str, value) {
  const indexWord = str.lastIndexOf(`${value}`);
  let newStr1 = '';
  let newStr2 = '';
  if (value === '') {
    return str;
...
function isPalindrome(str) {
  const lCaseStr = str
    ...
    .replace('?', '');
  let resultStr = '';
  for (let i = lCaseStr.length - 1; i >= 0; i -= 1) {
    if (lCaseStr[i] !== '!' || lCaseStr[i] !== '?' || lCaseStr[i] !== ',') {
      resultStr += lCaseStr[i];
    }
  }
  if (resultStr === lCaseStr) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  const newStrArr = sentence.split(' ');
  let resultStr = '';
  function searchLength(word) {
    if (resultStr === '' || word.length > resultStr.length) {
      resultStr = word;
    }
  }
  newStrArr.flatMap(searchLength);
  return resultStr;
}
...
function reverseWords(str) {
  ...
  function reverseWord(word) {
    ...
    return resultArr.push(resultStr);
  }
  newStrArr.forEach(reverseWord);
...
function invertCase(str) {
  const newStrArr = str.split(' ');
  const resultArr = [];
  const lang = 'qwertyuiopasdfghjklzxcvbnm';
  function reverseWord(word) {
    let resultStr = '';
    for (let i = 0; i <= word.length - 1; i += 1) {
      if (lang.includes(word[i])) {
        resultStr += word[i].toUpperCase();
      }
      if (!lang.includes(word[i])) {
        resultStr += word[i].toLowerCase();
      }
    }
    return resultArr.push(resultStr);
  }
...
function getStringFromTemplate(firstName, lastName) {
  const resultStr = `Hello, ${firstName} ${lastName}!`;
  return resultStr;
}
...
function encodeToRot13(str) {
  const wordStr = 'AaBbCcDdEeFfGgHhIiJjKkLlMm';
  const rot13Str = 'NnOoPpQqRrSsTtUuVvWwXxYyZz';
  let resultStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (wordStr.includes(str[i])) {
      resultStr += rot13Str[wordStr.search(str[i])];
    }
    if (rot13Str.includes(str[i])) {
      resultStr += wordStr[rot13Str.search(str[i])];
    }
